...
num_hiddens = 256
rnn_layer = nn.RNN(len(vocab), num_hiddens)     #nn.RNN有两个输入，其中训练数据是已经转为独热编码的三维张量

state = torch.zeros((1, batch_size, num_hiddens))       #（隐藏层数，批量大小，隐藏单元数）

# X = torch.rand(size=(num_steps, batch_size, len(vocab)))
# Y, state_new = rnn_layer(X, state)      #Y是每个时间步的隐状态，state_new是最后一个时间步的隐状态
...
# print(predict_ch8('hello', 10, net, vocab, device))
